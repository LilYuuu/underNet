...
int port = 1883;
char topic[] = "undnet/lily";
String clientID = "lilyClient";
...
void loop() {
  ...
  // reads the input from the light sensor
  ...
  if (!mqttClient.connected()) {
    ...
    if (millis() - lastSendTime >= sendInterval) {
      ...
      float lightness = float(rawAnalog) / 1024.0 * 100;

      String body = "{\"lightness\": ll}";
      body.replace("ll", String(lightness) + "%");
